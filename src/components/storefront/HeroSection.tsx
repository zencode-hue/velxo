...
export default function HeroSection() {
  return (
    <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden">
      {/* Background layers */}
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(245,158,11,0.18) 0%, transparent 60%)" }} />
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 50% 40% at 85% 80%, rgba(217,119,6,0.1) 0%, transparent 50%)" }} />
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 40% 35% at 10% 60%, rgba(251,191,36,0.06) 0%, transparent 50%)" }} />
      ...
      {/* Floating orbs */}
      <div className="absolute top-1/4 left-1/4 w-64 h-64 rounded-full opacity-5 blur-3xl" style={{ background: "radial-gradient(circle, #f59e0b, transparent)" }} />
      <div className="absolute bottom-1/3 right-1/4 w-48 h-48 rounded-full opacity-5 blur-3xl" style={{ background: "radial-gradient(circle, #d97706, transparent)" }} />
      ...
      {[
        ...
        <motion.div key={badge.text}
          ...
          style={{ background: "rgba(17,13,0,0.9)", border: "1px solid rgba(245,158,11,0.25)", backdropFilter: "blur(12px)" }}>
          <badge.icon size={12} className="text-amber-400" />
          {badge.text}
        </motion.div>
      ))}
      ...
      <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
          ...
          <Link href="/products"
            ...
            style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)", boxShadow: "0 4px 24px rgba(245,158,11,0.4)" }}>
          ...
          <Link href="/deals"
            ...
            style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.3)", backdropFilter: "blur(8px)" }}>
            <Zap size={16} className="text-amber-400" /> View Deals
          </Link>
        </motion.div>
