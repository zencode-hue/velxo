...
import { Upload, Package, ArrowLeft, RefreshCw, FileText } from "lucide-react";
...
export default function StaffInventoryPage() {
  ...
  const fetchStock = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/staff/inventory/${productId}`);
      if (res.ok) {
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        ...
        <button onClick={fetchStock} className="ml-auto btn-secondary text-sm px-3 py-2">
      ...
      {stockInfo && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="glass-card p-4 text-center">
            <div className="text-2xl font-bold text-white">{stockInfo.unlimitedStock ? "∞" : stockInfo.stockCount}</div>
      ...
      <div className="glass-card p-6 space-y-4">
        ...
        <button onClick={handleUpload} disabled={loading || !text.trim()} className="btn-primary text-sm px-6 py-2.5 gap-2">
          <Upload size={15} />
